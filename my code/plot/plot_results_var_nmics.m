...
valuearray = {'o',4, '--', 0.7, [0, 0.4470, 0.7410];'v', 5, '--',1.4,[0.9290, 0.6940, 0.1250];...
    'd',4,'--',1.5,[0.4940, 0.1840, 0.5560];'^',4,'--',1.2,[0.4660, 0.6740, 0.1880];...
    '>',4,'--',1, [0.4660, 0.6740, 0.1880]; 'x',5,'--',1,[0.3010,  0.7450, 0.9330];...
    'h',5,'--',1,[0.6350, 0.0780, 0.1840]};
...
for i = 1:nmethod
    ...
    for k = 1:length(sigma)
        ...
        ylim([0 200]);xlim([0.9, 4.1]);xticks(Nmics);
...
%    print(['../figures/',calib_type,'/var_mics_sub_scrores','.eps'], '-depsc');
