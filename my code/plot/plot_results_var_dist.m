...
valuearray = {'o', 4, '--', 0.7;'s', 5, '--', 0.9;'v', 5, '--',1.4;...
    'd',4,'--',1.5;'^',4,'--',1; '>',4,'--',1; 'x',5,'--',1;'h',5,'--',1};
...
% for src = 1:N
    ...
    subplot(221);
    h1(1) = plot(dis, mean(ops_ideal,2));grid on;hold on;
    subplot(222);
    h2(1) = plot(dis, mean(tps_ideal,2));grid on;hold on;
    subplot(223);
    h3(1) = plot(dis, mean(ips_ideal,2));grid on;hold on;
    subplot(224);
    h4(1) = plot(dis, mean(aps_ideal,2));grid on;hold on;
    lgd{1} = 'Ideal TF';
